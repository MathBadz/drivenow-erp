...
const sections = computed(() => {
    const list: { title: string; links: NavLink[] }[] = [
        { title: 'Main', links: [{ label: 'Dashboard', href: '/dashboard', icon: Gauge }] },
    ];
    if (isAdmin.value) {
        list.push({ title: 'Administration', links: [{ label: 'System Settings', href: '/settings/system', icon: Settings }] });
    }
    return list;
});
