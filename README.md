...
The platform is deployed on **Render** (compute) with **Neon** (managed PostgreSQL).

| Application | URL |
|-------------|-----|
| Operations Hub (admin) | `https://drivenow-bsit3b-auth.onrender.com` |
| Client Portal (public) | `https://drivenow-bsit3b-client.onrender.com` |

> Free-tier services sleep after a period of inactivity; the first request may take up to a
> minute to wake the service. *(If the deployment prefix differs, update the URLs above.)*
...
A full step-by-step guide for deploying to the free tiers of Render (compute) and Neon
(managed PostgreSQL) is provided in **[`render/DEPLOY_RENDER.md`](render/DEPLOY_RENDER.md)**.
In summary:

1. Create one Neon project and run `render/neon-create-databases.sql` to create the eight databases.
2. Run `render/generate-render-env.py` to produce the per-service environment files.
3. Create eight Render web services (Docker runtime, one per `*-service` directory) and paste
   the matching environment file into each.
4. Database tables and demo data are created automatically by the container entrypoint
   (`php artisan migrate --force` + seeding) on first deploy.
...
## Note on Repository History and Deployment
...
and notes, and also handled the containerization and cloud deployment.

For this reason, the recent Git commit history and the live deployment are concentrated under
a single member's account. This does **not** reflect the actual division of work: the project
was a team effort throughout, with the System Analyst leading the business/domain analysis and
data design, Quality Assurance verifying every module and workflow, the Researcher covering the
technology stack, security and deployment options, and the Project Manager coordinating scope,
schedule and deliverables (see the team table above and the weekly progress report submitted
with this project).
